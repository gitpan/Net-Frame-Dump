#
# $Id: Dump.pm,v 1.3 2006/12/03 16:21:22 gomor Exp $
#
...
sub new {
   my $self = shift->SUPER::new(
      file       => "netframe-tmp-$$.@{[getRandom32bitsInt()]}.pcap",
      filter     => '',
      overwrite  => 0,
      timeout    => 0,
      promisc    => 0,
      ...
      keepTimestamp  => 0,
      _sDataAwaiting => 0,
      _sName         => "netframe-tmp-$$.@{[getRandom32bitsInt()]}.storable",
      @_,
   );

   unless ($self->[$__dev]) {
      confess("You MUST set `dev' attribute\n");
   }
...
sub _getPcapHeader {
   my $self = shift;
   my $link = $self->[$__link];
   # XXX: be able to handle other than DLT_RAW
   # 24 bytes header of a DLT_RAW (0x0c) pcap file
   "\xd4\xc3\xb2\xa1\x02\x00\x04\x00\x00\x00\x00\x00".
   "\x00\x00\x00\x00\xdc\x05\x00\x00".pack('C', $link)."\x00\x00\x00";
}
...
sub store {
   ...
   $self->framesStored($oSimple);
   push @{$self->[$__frames]}, $oSimple;
}
...
sub _pcapNext {
   ...
   if (my $raw = Net::Pcap::next($self->[$___pcapd], \%hdr)) {
      my $ts = $self->[$__keepTimestamp] ? $self->_getTimestamp(\%hdr)
                                         : $self->_setTimestamp;
      return {
         firstLayer => $mapLinks->{$self->[$__link]} || 'UNKNOWN',
         timestamp  => $ts,
         raw        => $raw,
      };
   }
...
sub _getNextAwaitingFrameOffline { shift->_pcapNext }
...
sub getFramesFor {
   ...
   my $aref = $self->[$__framesStored]->{$key};
...
sub framesStored {
   ...
   confess("Usage: framesStored(Net::Frame::Simple object)\n")
      unless $frame;

#  unless (my $ref = ref($frame) && $ref eq 'Net::Frame::Simple') {
#     carp("Can only take a Net::Frame::Simple object\n");
#     return undef;
#  }

   if ($frame) {
      my $key = $frame->getKey;
      push @{$self->[$__framesStored]->{$key}}, $frame;
   }

#  print "XXX: Dump::framesStored: ICMP storing may be broken for now\n";
